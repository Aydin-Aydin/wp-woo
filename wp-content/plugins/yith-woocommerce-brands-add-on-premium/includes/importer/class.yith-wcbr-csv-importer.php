...
if ( ! class_exists( 'YITH_WCBR_CSV_Importer' ) ) {
	...
	class YITH_WCBR_CSV_Importer extends WP_Importer {
		...
		public function import( $file ) {
			if ( ! is_file( $file ) ) {
				$this->import_error( __( 'The file does not exist, please try again.', 'yith-wcbr' ) );
			}
						...
						if ( ! is_wp_error( $id ) ) {
							...
							if ( ! empty( $term_thumbnail ) ) {
								...
								if ( ! is_wp_error( $attachment_id ) ) {
									...
									echo sprintf( __( 'Failed to import attachment %s', 'yith-wcbr' ), esc_html( $term_thumbnail ) );
									echo '<br />';
								}
							}
							...
							echo sprintf( __( 'Failed to import %s %s', 'yith-wcbr' ), esc_html( YITH_WCBR::$brands_taxonomy ), esc_html( $term_name ) );
							echo '<br />';
						}
					}

				} else {
					$this->import_error( __( 'The CSV is invalid.', 'yith-wcbr' ) );
				}
			...
			// Show Result
			echo '<div class="updated settings-error below-h2"><p>' . sprintf( __( 'Import complete - imported <strong>%s</strong> brands.', 'yith-wcbr' ), $loop ) . '</p></div>';
		...
		public function import_end() {
			echo '<p>' . __( 'All done!', 'yith-wcbr' ) . ' <a href="' . esc_url( add_query_arg( array( 'taxonomy'  => YITH_WCBR::$brands_taxonomy,
			                                                                                            'post_type' => 'product'
				), admin_url( 'edit-tags.php' ) ) ) . '">' . __( 'View Brands', 'yith-wcbr' ) . '</a>' . '</p>';
		...
		public function header() {
			...
			echo '<h2>' . __( 'YITH Brands Importer', 'yith-wcbr' ) . '</h2>';
		}
		...
		public function greet() {
			...
			echo '<p>' . __( 'Hi there! Upload a CSV file containing brands to import the contents into your shop. Choose a .csv file to upload, then click "Upload file and import".', 'yith-wcbr' ) . '</p>';
			...
			if ( ! empty( $upload_dir['error'] ) ) :
				?>
				<div class="error">
				<p><?php _e( 'Before uploading your import file, you need to fix the following error:', 'yith-wcbr' ); ?></p>
				...
				<form enctype="multipart/form-data" id="import-upload-form" method="post" action="<?php echo esc_attr( wp_nonce_url( $action, 'import-upload' ) ); ?>">
					<table class="form-table">
						...
						<tr>
							<th>
								<label for="upload"><?php _e( 'Choose a file from your computer:', 'yith-wcbr' ); ?></label>
							</th>
							<td>
								<input type="file" id="upload" name="import" size="25" />
								<input type="hidden" name="action" value="save" />
								<input type="hidden" name="max_file_size" value="<?php echo $bytes; ?>" />
								<small><?php printf( __( 'Maximum size: %s', 'yith-wcbr' ), $size ); ?></small>
							</td>
						</tr>
						<tr>
							<th>
								<label for="file_url"><?php _e( 'OR enter path to file:', 'yith-wcbr' ); ?></label>
							</th>
							<td>
								<?php echo ' ' . ABSPATH . ' '; ?>
						...
						<tr>
							<th><label><?php _e( 'Delimiter', 'yith-wcbr' ); ?></label><br /></th>
							<td><input type="text" name="delimiter" placeholder="," size="2" /></td>
						</tr>
						</tbody>
					</table>
					<p class="submit">
						<input type="submit" class="button" value="<?php esc_attr_e( 'Upload file to import', 'yith-wcbr' ); ?>" />
					</p>
				</form>
			<?php
			endif;
